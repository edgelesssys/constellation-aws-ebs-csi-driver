...
func NewNodeService(o *Options, md metadata.MetadataService, m mounter.Mounter, k kubernetes.Interface, cm cryptMapper) *NodeService {
...
func (d *NodeService) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (_ *csi.NodeStageVolumeResponse, retErr error) {
	...
	cryptDevicePath, err := d.cm.OpenCryptDevice(ctx, source, volumeID, integrity)
	if err != nil {
	...
	defer func() {
		if retErr != nil {
			...
			if err := d.cm.CloseCryptDevice(volumeID); err != nil {
	...
	klog.V(4).InfoS("NodeStageVolume: formatting and mounting with fstype", "source", source, "devicePath", devicePath, "volumeID", volumeID, "target", target, "fstype", fsType)
	...
	err = d.mounter.FormatAndMountSensitiveWithFormatOptions(cryptDevicePath, target, fsType, mountOptions, nil, formatOptions)
	if err != nil {
		msg := fmt.Sprintf("could not format %q and mount it at %q: %v", cryptDevicePath, target, err)
		return nil, status.Error(codes.Internal, msg)
	}

	needResize, err := d.mounter.NeedResize(cryptDevicePath, target)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Could not determine if volume %q (%q) need to be resized:  %v", req.GetVolumeId(), cryptDevicePath, err)
	}

	if needResize {
		klog.V(2).InfoS("Volume needs resizing", "cryptDevicePath", cryptDevicePath)
		if _, err := d.mounter.Resize(cryptDevicePath, target); err != nil {
			return nil, status.Errorf(codes.Internal, "Could not resize volume %q (%q):  %v", volumeID, source, err)
		}
	}
	klog.V(4).InfoS("NodeStageVolume: successfully formatted and mounted volume", "source", source, "devicePath", devicePath, "volumeID", volumeID, "target", target, "fstype", fsType)
	return &csi.NodeStageVolumeResponse{}, nil
}
...
func (d *NodeService) NodeExpandVolume(ctx context.Context, req *csi.NodeExpandVolumeRequest) (*csi.NodeExpandVolumeResponse, error) {
	...
	if volumeCapability != nil {
		...
		devicePath, err = d.cm.ResizeCryptDevice(ctx, volumeID)
		if err != nil {
